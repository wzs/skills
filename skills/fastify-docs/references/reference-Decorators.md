...
This API is *synchronous*. Defining a decoration asynchronously could result in the Fastify instance booting before the decoration completes. To register an asynchronous decoration, use the `register` API with `fastify-plugin`. See the [Plugins](reference-Plugins.md) documentation for more details.
...
This method customizes the Fastify [server](reference-Server.md) instance.
...
The decorated [Fastify server](reference-Server.md) is bound to `this` in [route](reference-Routes.md) handlers:
...
To achieve proper encapsulation across requests configure a new value for each incoming request in the [`'onRequest'` hook](reference-Hooks.md#onrequest).
...
> ℹ️ Note: For TypeScript users, `getDecorator` supports generic type parameters. See the [TypeScript documentation](reference-TypeScript.md) for advanced typing examples.
...
> ℹ️ Note: For TypeScript users, see the [TypeScript documentation](reference-TypeScript.md) for advanced typing examples using `setDecorator<T>`.
