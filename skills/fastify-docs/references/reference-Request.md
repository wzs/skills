...
* `body` - The request payload, see [Content-Type Parser](reference-ContentTypeParser.md) for details on what request payloads Fastify natively parses and how to support other content types.
...
* `host` - The host of the incoming request (derived from `X-Forwarded-Host` header when the [`trustProxy`](reference-Server.md#factory-trust-proxy) option is enabled). For HTTP/2 compatibility, it returns `:authority` if no host header exists. The host header may return an empty string if `requireHostHeader` is `false`, not provided with HTTP/1.0, or removed by schema validation.
...
* `context` - Deprecated, use `request.routeOptions.config` instead. A Fastify internal object. Do not use or modify it directly. It is useful to access one special key:
  <!-- -->
  * `context.config` - The route [`config`](reference-Routes.md#routes-config) object.

* `routeOptions` - The route [`option`](reference-Routes.md#routes-options) object.

  <!-- -->

  * `bodyLimit` - Either server limit or route limit.
  * `config` - The [`config`](reference-Routes.md#routes-config) object for this route.
...
This function compiles a validation schema and returns a function to validate data. The returned function (a.k.a. *validation function*) is compiled using the provided [`SchemaController#ValidationCompiler`](reference-Server.md#schema-controller). A `WeakMap` is used to cache this, reducing compilation calls.
