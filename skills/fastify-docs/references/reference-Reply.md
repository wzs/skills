...
> **Note:** Even when the `Content-Type` header is set to `application/json`, strings are sent unmodified by default. To serialize a string as JSON, you must set a custom serializer:
...
See special note about error handling for streams in [`setErrorHandler`](reference-Server.md#seterrorhandler).
...
Beware that calling `reply.send(error)` in your custom error handler will send the error to the default error handler. Check out the [Reply Lifecycle](reference-Lifecycle.md#reply-lifecycle) for more information.
...
The type of the sent payload (after serialization and going through any [`onSend` hooks](reference-Hooks.md#onsend)) must be one of the following types, otherwise, an error will be thrown:
...
If you want to know more please review [Routes#async-await](reference-Routes.md#async-await).
