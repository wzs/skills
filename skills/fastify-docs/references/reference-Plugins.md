...
Fastify can be extended with plugins, which can be a set of routes, a server [decorator](reference-Decorators.md), or other functionality. Use the `register` API to add one or more plugins.
...
The [Getting Started](guides-Getting-Started.md#your-first-plugin) guide includes an example of using this API:
...
* [`logLevel`](reference-Routes.md#custom-log-level)
* [`logSerializers`](reference-Routes.md#custom-log-serializer)
...
The Fastify instance passed to the function is the latest state of the **external Fastify instance** the plugin was declared on, allowing access to variables injected via [`decorate`](reference-Decorators.md) by preceding plugins according to the **order of registration**. This is useful if a plugin depends on changes made to the Fastify instance by a preceding plugin, such as utilizing an existing database connection.
...
If an option with the key `prefix` and a `string` value is passed, Fastify will use it to prefix all the routes inside the register. For more info, check [here](reference-Routes.md#route-prefixing).

Be aware that if routes are wrapped with [`fastify-plugin`](https://github.com/fastify/fastify-plugin), this option will not work (see the [workaround](reference-Routes.md#fastify-plugin)).
...
If `register` is used only to extend server functionality with [`decorate`](reference-Decorators.md), tell Fastify not to create a new scope. Otherwise, changes will not be accessible in the upper scope.
