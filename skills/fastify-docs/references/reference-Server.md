...
Defines the maximum payload, in bytes, the server is allowed to accept. The default body reader sends [`FST_ERR_CTP_BODY_TOO_LARGE`](reference-Errors.md#fst_err_ctp_body_too_large) reply, if the size of the body exceeds this limit. If [`preParsing` hook](reference-Hooks.md#preparsing) is provided, this limit is applied to the size of the stream the hook returns (i.e. the size of "decoded" body).
...
Defines what action the framework must take when parsing a JSON object with `__proto__`. This functionality is provided by [secure-json-parse](https://github.com/fastify/secure-json-parse). See [Prototype Poisoning](guides-Prototype-Poisoning.md) for more details about prototype poisoning attacks.
...
Defines what action the framework must take when parsing a JSON object with `constructor`. This functionality is provided by [secure-json-parse](https://github.com/fastify/secure-json-parse). See [Prototype Poisoning](guides-Prototype-Poisoning.md) for more details about prototype poisoning attacks.
...
You may access the `ip`, `ips`, `host` and `protocol` values on the [`request`](reference-Request.md) object.
...
Configure the Ajv v8 instance used by Fastify without providing a custom one. The default configuration is explained in the [#schema-validator](reference-Validation-and-Serialization.md#schema-validator) section.
...
> **Note** The `req` and `res` objects passed to `defaultRoute` are the raw Node.js `IncomingMessage` and `ServerResponse` instances. They do **not** expose the Fastify-specific methods available on `FastifyRequest`/`FastifyReply` (for example, `res.send`).
...
Method to add routes to the server, it also has shorthand functions, check [here](reference-Routes.md).
...
`fastify.close(callback)`: call this function to close the server instance and run the [`'onClose'`](reference-Hooks.md#on-close) hook.
...
Fastify allows the user to extend its functionality with plugins. A plugin can be a set of routes, a server decorator, or whatever, check [here](reference-Plugins.md).

#### addHook[​](#addhook "Direct link to addHook")

[]()

Function to add a specific hook in the lifecycle of Fastify, check [here](reference-Hooks.md).
...
The logger instance, check [here](reference-Logging.md).

#### version[​](#version "Direct link to version")

[]()

Fastify version of the instance. Used for plugin support. See [Plugins](reference-Plugins.md#handle-the-scope) for information on how the version is used by plugins.
...
To learn more, read the [Validation and Serialization](reference-Validation-and-Serialization.md) documentation.
...
Set the reply serializer for all the routes. This will be used as default if a [Reply.serializer(func)](reference-Reply.md#serializerfunc) has not been set. The handler is fully encapsulated, so different plugins can set different error handlers. Note: the function parameter is called only for status `2xx`. Check out the [`setErrorHandler`](#seterrorhandler) for errors.
...
Set the schema validator compiler for all routes. See [#schema-validator](reference-Validation-and-Serialization.md#schema-validator).

#### setSchemaErrorFormatter[​](#setschemaerrorformatter "Direct link to setSchemaErrorFormatter")

[]()

Set the schema error formatter for all routes. See [#error-handling](reference-Validation-and-Serialization.md#schemaerrorformatter).

#### setSerializerCompiler[​](#setserializercompiler "Direct link to setSerializerCompiler")

[]()

Set the schema serializer compiler for all routes. See [#schema-serializer](reference-Validation-and-Serialization.md#schema-serializer).
...
This property can be used to set a function to format errors that happen while the `validationCompiler` fails to validate the schema. See [#error-handling](reference-Validation-and-Serialization.md#schemaerrorformatter).
  ...
  schemaController: {
    ...
    compilersFactory: {
      ...
      buildValidator: function factory (externalSchemas, ajvServerOption) {
        ...
        // See [#schema-validator](reference-Validation-and-Serialization.md#schema-validator) for details.
      ...
      buildSerializer: function factory (externalSchemas, serializerOptsServerOption) {
        ...
        // See [#schema-serializer](reference-Validation-and-Serialization.md#schema-serializer) for details.
...
`fastify.setNotFoundHandler(handler(request, reply))`: set the 404 handler. This call is encapsulated by prefix, so different plugins can set different not found handlers if a different [`prefix` option](reference-Plugins.md#route-prefixing-option) is passed to `fastify.register()`. The handler is treated as a regular route handler so requests will go through the full [Fastify lifecycle](reference-Lifecycle.md#lifecycle). *async-await* is supported as well.

You can also register [`preValidation`](reference-Hooks.md#route-hooks) and [`preHandler`](reference-Hooks.md#route-hooks) hooks for the 404 handler.

> ℹ️ Note: The `preValidation` hook registered using this method will run for a route that Fastify does not recognize and **not** when a route handler manually calls [`reply.callNotFound`](reference-Reply.md#call-not-found). In which case, only preHandler will be run.
...
`fastify.setErrorHandler(handler(error, request, reply))`: Set a function that will be invoked whenever an exception is thrown during the request lifecycle. The handler is bound to the Fastify instance and is fully encapsulated, so different plugins can set different error handlers. *async-await* is supported as well.
...
`fastify.printRoutes({ includeMeta: (true | []) })` will display properties from the `route.store` object for each displayed route. This can be an `array` of keys (e.g. `['onRequest', Symbol('key')]`), or `true` to display all properties. A shorthand option, `fastify.printRoutes({ includeHooks: true })` will include all [hooks](reference-Hooks.md).
...
For more details about the usage of the different content type parser APIs see [here](reference-ContentTypeParser.md#usage).
